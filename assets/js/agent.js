...
function getActiveIdentity() {
  return identities.find((record) => record.identityId === activeIdentityId) ?? null;
}
...
function renderProofOutput() {
  const identity = getActiveIdentity();
  if (!identity || !identity.proofBundle) {
    proofOutputEl.textContent = "";
    proofExportEl.value = "";
    btnCopyProof.disabled = true;
    return;
  }
  const payload = JSON.stringify(identity.proofBundle, null, 2);
...
btnCreateIdentity.addEventListener("click", async () => {
  ...
  try {
    ...
    console.error(err);
    identityStatusEl.textContent = err instanceof Error ? err.message : "Error creating identity.";
  } finally {
...
btnGenerateProof.addEventListener("click", async () => {
  ...
  try {
    const proof = await requestProof(identity.identityId, identity.commitment);
    identity.proofBundle = {
      identityId: identity.identityId,
      proof,
    };
...
btnCopyIdentity.addEventListener("click", async () => {
  const payload = getIdentityExportPayload(getActiveIdentity());
  if (!payload) return;
  try {
...
btnCopyProof.addEventListener("click", async () => {
  ...
  const proofBundle = activeIdentity ? activeIdentity.proofBundle : null;
